...
export function classifyFile(path: string): string {
  ...
  if (
    ...
    /_(test|spec)\.(go|py|rb|exs?)$/.test(base) ||
    /^test_[^/]*\.py$/.test(base) ||
    p.includes('__tests__/') ||
    p.includes('__mocks__/') ||
    /(^|\/)tests?\//.test(p) ||
    // spec/ dirs are RSpec only for Ruby files — OpenAPI specs live there too
    (/(^|\/)specs?\//.test(p) && base.endsWith('.rb'))
  )
    return 'test'
...
/** Below this many bytes a file produces too few findings to be worth excluding. */
const MIN_MINIFIED_BYTES = 2_000
/**
 * AVERAGE line width separating packed build output from authored code.
 * Measured across real repos: authored files top out around 62 chars/line
 * (a 639-line React/Tailwind route) and 27 (a 50KB app.js), while every
 * minified bundle observed started at 2,556 (jQuery 16,849; Ace 22,069).
 *
 * Deliberately NOT max line width: real React/Tailwind source carries the odd
 * 1,000+ char className string or inline SVG path, and excluding a whole file
 * for one long line silently hides authored code from every analyzer.
 */
const MINIFIED_MEAN_LINE_WIDTH = 200

/**
 * Minified/bundled output: a build artifact or vendored library, not authored
 * source. Analyzing it reports "deep nesting" and "oversized file" against code
 * the team never wrote and cannot fix — on one real audit these were 41% of all
 * findings. Detected by shape rather than filename so bundles that are not
 * named `.min.js` are caught too.
 */
export function looksMinified(content: string): boolean {
  if (content.length < MIN_MINIFIED_BYTES) return false
  let newlines = 0
  for (let i = 0; i < content.length; i++) {
    if (content.charCodeAt(i) === 10 /* \n */) newlines++
  }
  return content.length / (newlines + 1) >= MINIFIED_MEAN_LINE_WIDTH
}
