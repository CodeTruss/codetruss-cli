...
import type { InferredScopeBasis, Receipt } from './types.js'

/**
 * Every byte a receipt renders, including the frozen per-profile blocks.
 *
 * These bytes sit inside signed receipts, so a superseded wording can never be
 * edited — only added to. `verifyReceipt` re-renders a receipt through the
 * variant that could have produced it and compares, which is why the historical
 * renderers below still exist and must stay byte-exact.
 */
