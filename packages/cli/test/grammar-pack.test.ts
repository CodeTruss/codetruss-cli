 ...
 * platform.
 *
 * `grammarDataDir` reads LOCALAPPDATA on Windows and XDG_DATA_HOME everywhere
 * else, so setting one alone isolates one platform and leaves the other pointed
 * at the developer's real data directory — where this suite installs a 722 KB
 * pack, tampers with it mid-suite, and abandons it.
 */
...
/**
 * Run `body` with `process.platform` pinned.
 *
 * `grammarDataDir` branches on the platform, so an unpinned assertion about one
 * branch only asserts anything on the machine that happens to run it. Pinning is
 * what makes the POSIX expectations below mean the same thing on Windows as they
 * do here, instead of quietly asserting the other branch's behavior.
 */
...
describe('grammar pack storage location', () => {
  it('puts packs under XDG_DATA_HOME, not the config directory', () => {
    const dir = onPlatform('linux', () => grammarDataDir({ XDG_DATA_HOME: '/data' }))
  ...
  it('refuses a relative XDG_DATA_HOME rather than resolving it against the cwd', () => {
    expect(() => onPlatform('linux', () => grammarDataDir({ XDG_DATA_HOME: 'relative/path' })))
      .toThrow(/absolute/)
  })
...
describe('the pinned manifest', () => {
  ...
  it('derives the on-disk directory from the pack version, so packs coexist', () => {
    const pack = pinnedGrammarPack('python')!
    expect(onPlatform('linux', () => grammarPackDir(pack, { XDG_DATA_HOME: '/data' })))
